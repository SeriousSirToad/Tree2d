...
public class Player extends Mob {
	...
	public Player(String username, int x, int y, InputHandler input, Island is) {

		super(x, y, "player", Colour.fixYoSelf(colors, newColors, Assets.PLAYER), is);
		name = username;
	...
	public void render(Graphics g) {
		...
		if (moving) {
			if (movingDir == 1) {
				g.drawImage(walkR.animate(), x - GameState.camera.x(), y - GameState.camera.y(), width, height, null);
			} else if (movingDir == 3) {
				g.drawImage(walkR.animate(), x - GameState.camera.x() + width, y - GameState.camera.y(), -width, height, null);
			} else if (movingDir == 2) {
				g.drawImage(walkD.animate(), x - GameState.camera.x(), y - GameState.camera.y(), width, height, null);
			} else {
				g.drawImage(walkU.animate(), x - GameState.camera.x(), y - GameState.camera.y(), width, height, null);
			}
		} else {
			// System.out.println("penis");
			switch (movingDir) {
			case 1:
				g.drawImage(walkR.currentFrame, x - GameState.camera.x(), y - GameState.camera.y(), width, height, null);
				walkR.reset();
				break;
			case 3:
				g.drawImage(walkR.currentFrame, x - GameState.camera.x() + width, y - GameState.camera.y(), -width, height, null);
				walkR.reset();
				break;
			case 2:
				g.drawImage(walkD.currentFrame, x - GameState.camera.x(), y - GameState.camera.y(), width, height, null);
				walkD.reset();
				break;
			case 4:
				g.drawImage(walkU.currentFrame, x - GameState.camera.x(), y - GameState.camera.y(), width, height, null);
				walkU.reset();
				break;
			default:
				g.drawImage(walkR.currentFrame, x - GameState.camera.x(), y - GameState.camera.y(), width, height, null);
